...
const GamesList = () => {
  ...
  return (
    <section className="pt-0 pb-4 md:pb-16 relative min-h-[calc(100vh-7.5rem)]" id="games">
      ...
      <div className="container mx-auto px-4">
        <div className="relative rounded-3xl border border-white/10 bg-black/30 backdrop-blur-md p-4 md:p-6 shadow-[0_0_28px_rgba(57,255,20,0.12)]">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2.5 sm:gap-3">
            {filteredGames.map((game) => (
              <motion.div
                key={game.id}
                className="bg-charcoal-800/80 rounded-xl overflow-hidden group border border-electric-500/20 shadow-[0_0_20px_rgba(154,47,255,0.15)]"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
